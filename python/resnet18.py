...
def draw(origimg, rectangles, boxNums): 
    ...
    for i in range(boxNums):
        if rectangles[i * 7 + 1] == -1:
            ...
            if rectangles[i * 7 + 1] == 1:
                print("X: {},{} Y: {},{}".format(int(rectangles[i*7+3]*W),int(rectangles[i*7+4]*H),int(rectangles[i*7+5]*W),int(rectangles[i*7+6]*H)))
...
if __name__ == '__main__':
    ...
    #inputs = torch.ones(inputs.shape[0],inputs.shape[1],inputs.shape[2],inputs.shape[3]) * 128
    result = net(inputs)
    pdb.set_trace()
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network() as trt_network:
        ...
        cpu_arm_loc = trt_outputs[0].host
        cpu_arm_conf = trt_outputs[1].host
        cpu_odm_loc = trt_outputs[2].host
        cpu_odm_conf = trt_outputs[3].host
        prior_fine = trt_outputs[4].host
        conf_fine = trt_outputs[5].host
        trt_outputs = trt_outputs[6].host
        pdb.set_trace()
